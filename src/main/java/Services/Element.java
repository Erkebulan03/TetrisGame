...
private int ticks;
Mapable map;
    public Element (Mapable map) {
        this.map=map;
        figure= Figures.getRandom();
        coord= new Coord(Cunfig.WIDTH/ 2 - 2, -1);
        landed= false;
        ticks=2;
    }
...
public boolean isLanded() {
    ...
    private boolean canMoveFigure(Figures figure, int sx, int sy) {
        ...
        if(coord.x+sx+figure.bot.x>= Cunfig.WIDTH) return false;
        //if(coord.y +sy +figure.top.y<0) return false;
        if(coord.y + sy +figure.bot.y>=Cunfig.HEIGHT) return false;
